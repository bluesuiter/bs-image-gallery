...
class bsImageGalleryAdminInterface extends bsDataClass {
    ...
    public function bsGalleryList() {
        ...
        ?>
        <div class="wrap">
            <?php if (isset($_COOKIE['glryMsg'])) { ?>
                <p style="color: #0056b3"><?php
                    echo $_COOKIE['glryMsg'];
                    unset($_COOKIE['glryMsg']);
                    ?></p>
                <?php
            }
            ?>
    ...
    public function bsEditGalleryForm() {
        ...
        $imgURL = checkArrayValue($imageData, 'img_id');
        $imgTitle = checkArrayValue($imageData, 'img_title');
        $imgDesc = checkArrayValue($imageData, 'img_desc');
        ?>
        <div class="col-1">
            <div class="col-3">
            ...
            <div class="col-3">
                ...
                <span class="thumb-prev-cont">
                    <?php $thumbUrl = wp_get_attachment_image_src($thumbNail)[0]; ?>
                    <img src="<?php echo $thumbUrl; ?>" alt="" id="thumbnail-prev" style="max-width:100%;"/>
                </span>
        ...
        <table class="wp-list-table widefat fixed striped posts" cellspacing="0" id="ivmGallery">
            ...
            <tbody id="ivmGalleryBody">
                <?php for ($ika = 0; $ika < count($imgURL); $ika++) { ?>
                    <?php $itemUrl = wp_get_attachment_image_src($imgURL[$ika])[0] ?>
                    <tr class="connectedSortable" id="bsigrow<?php echo $ika ?>" >
                        <td>
                            <img style="max-width:100px;" src="<?php echo $itemUrl ?>" class="previmg" id="previmg<?php echo $ika ?>"/>
